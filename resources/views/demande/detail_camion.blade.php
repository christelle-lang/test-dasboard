...
@endif
  ...
  <div class="main_content_iner ">
    ...
    <div class="row mb-4">
    <div class="col">
        <h4 class="m-0">Information du camion</h4><br>
        <div>
    <strong></strong>
    </div>
    <div>Type du camion: {{$camion->typeCamion}}</div><hr>
    <div>Caractéristique du camion: {{$camion->caractéristiqueCamion}}</div><hr>
    
    <div>Numérod'immatriculation: {{$camion->numImmatriculation}}</div><hr>
    <div>Capacité de charge: {{$camion->capaciteDeCharge}}</div><hr>
    <div>Photo du camion: {{$camion->photoCamion}} </div><hr>
    <div>Statut: {{$camion->statut}} </div><hr>
