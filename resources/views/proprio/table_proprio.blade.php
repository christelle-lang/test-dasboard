...
<h5 class="m-0">Liste des propriétaires</h5>
</div>
...
<div class="search_inner">
    <form action="{{route('recherche_proprio')}}" method="get">
        @csrf
       ...
       <div class="QA_table mb_30">
        @if(isset($table_proprio) && isset($query))
    ...
    @forelse($table_proprio as $table_proprios)
        ...
        @endif
                <tr>
                    <td>{{ $loop->index + 1 }}</td>
                    <td>{{ $table_proprios->nomProprio  }}</td>
                    <td>{{ $table_proprios->prenomProprio }}</td>
                    <td>{{ $table_proprios->emailProprio  }}</td>
                    <td>{{ $table_proprios->numTelProprio }}</td>
                    <td>{{ $table_proprios->numTelProprio2 }}</td>
                    
                    @if ($table_proprios->statut == 'Activé')
                    ...
                    <td>
                        <div class="action_btns d-flex">
                            <a href="{{ route('modifier_proprios',['id'=>$table_proprios->id]) }}" class="action_btn"> <i class=" far fa-edit "></i> </a>
                            <a href="{{ route('supprimer_proprios',['id'=>$table_proprios->id]) }}" class="action_btn  "> <i class="fas fa-trash"></i> </a>
                            <a href="{{ route('detail_proprios',['id'=>$table_proprios->id]) }}" class="action_btn  "> <i class="fa-solid fa-eye"></i></a>
...
<div class="footer_part">
    ...
    <div class="footer_iner text-center">
           ...
           <div class="modal fade" id="ajouterDemande" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" >
            ...
            <h5 class="modal-title" id="exampleModalLongTitle">Ajouter un propriétaire</h5>
